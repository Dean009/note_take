BEGIN WORKING ON CONTAINERS

Add buttons to each entered list item.
Add a refresh list, that deleted all the items. 
Save the list - Saving works, need to chage so its a Save As option with override. Need to space the list out in the text file better.  
Make it so it automatically loads the last opened list. 
Change the design.
Calander so can assign the items to a day of the week. 
Box to choose which save to load. 
Asked on launch if you would like to launch an old save.
Font options.
Email support. - emailing the list to selected email address, add a list of regular email addresses?
Implement google login. 
Icons/logo/backgrounds. 
Mulitple user support?
Seperate work/social/play lists.
Voice functionality?
App? - How to access it? on main lock screen?
Sharing lists
Edit Item is list
